...
We always reset the DB on container start so the PoC starts in a known state.
Two users:
  alice@example.com / Senha123  (owner of several requests, including ones to cancel)
  bob@example.com   / Senha123  (owner of a couple, used to test ownership rules)

Designed to exercise US-04 filters (multiple statuses, multiple priorities)
and US-05 cancellation rules (only 'aberta'/'em_analise' may be cancelled).
"""
...
def _seed_requests(db, alice_id: int, bob_id: int) -> None:
    now = datetime.utcnow()
    rows = [
        # Alice — various statuses + priorities (good for filter tests)
        ServiceRequest(
        ...
        # Bob — used to verify ownership rules in US-04 and US-05
        ServiceRequest(
